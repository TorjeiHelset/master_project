...
class NetworkState:
    ...
    def __init__(self, network):
        ...
        for road in network.roads:
            ...
            road_dict["left_bndry"] = road.left_boundary.detach().clone()
            road_dict["right_bndry"] = road.right_boundary.detach().clone()
            road_dict["left_flux"] = road.left_flux.detach().clone()
            road_dict["right_flux"] = road.right_flux.detach().clone()
    ...
    def copy_to_network(self, network):
        for road, road_state in zip(network.roads, self.road_state):
            ...
            road.left_boundary = road_state["left_bndry"]
            road.right_boundary = road_state["right_bndry"]
            road.left_flux = road_state["left_flux"]
            road.right_flux = road_state["right_flux"]
...
class GeneralRestartingRoadNetwork:
    '''
    Specific class for performing simulating a network for kvadraturen with e18, creating a new
    RoadNetwork class every time a new bus stop is reached
    '''

    def __init__(self, T, N, speed_limits, control_points, cycle_times, config, type):
        self.T = T
        self.N = N
        self.speed_limits = speed_limits
        self.control_points = control_points
    ...
    def _first_step(self):
        match self.type:
            case 2:
                # 2x2 junction
                network = generate.two_two_junction(self.T, self.N, self.speed_limits, self.control_points,
                                                    self.cycle_times[0], track_grad=True)
            case 3:
                # Medium complex
                network = generate.medium_complex_network(self.T, self.N, self.speed_limits, self.control_points,
                                                    self.cycle_times, track_grad=True)
            case _:
                raise NotImplementedError(f"Restarting network for this case not implemented yet!")
