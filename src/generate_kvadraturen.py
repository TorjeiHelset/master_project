...
def create_roads_minimal_junctions_for_roundabout(N = 2, v_strand_speeds = [torch.tensor(50.0)], v_strand_controls = [],
    ...
    L = 25 # Length of road
    # N: Number of cells to be used for every L meters, In addition to these internal cells, every road has 2*order ghost cells
    offset = 0.1
    tilt = 0.025
    # Vestre Strandgate:
...
def create_roundabouts(v_strand_fw, v_strand_bw, festning_fw, festning_bw,
                       speed1 = [torch.tensor(50.0)], speed2 = [torch.tensor(50.0)], N = 2,
                       track_grad = True):
