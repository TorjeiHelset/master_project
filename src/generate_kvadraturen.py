...
def create_roads_w_e18(N = 2, v_strand_speeds = [torch.tensor(50.0)], v_strand_controls = [],
                        h_w_speeds = [torch.tensor(30.0)], h_w_controls = [],
                        tollbod_speeds = [torch.tensor(30.0)], tollbod_controls = [],
                        elvegate_speeds = [torch.tensor(30.0)], elvegate_controls = [],
                        dronning_speeds = [torch.tensor(50.0)], dronning_controls = [],
                        festning_speeds = [torch.tensor(50.0)], festning_controls = [],
                        lundsbro_speeds = [torch.tensor(50.0)], lundsbro_controls = [],
                        e18_speeds = [torch.tensor(80.0)], e18_controls = [],
                        e18_w_1_speeds = [torch.tensor(60.0)], e18_w_1_controls= [],
                        e18_w_2_speeds = [torch.tensor(60.0)], e18_w_2_controls= [],
                        e18_e_1_speeds = [torch.tensor(60.0)], e18_e_1_controls= [],
                        e18_e_2_speeds = [torch.tensor(60.0)], e18_e_2_controls= [],
                        init_densities = None, boundary_configs = None,
                        track_grad = True):
    ...
    # Creating functions for the initial distribution of densities
    if init_densities is None:
        initial_fncs = [lambda x : torch.ones_like(x) * 0.2 for _ in range(12)]
    else:
        initial_fncs = [ibc.init_density(init_densities[i], 1) for i in range(12)]
    ...
    if boundary_configs is None:
        ...
        lundsbro_bndry = ibc.boundary_conditions(1, max_dens = 2, densities = torch.tensor([0.15, 0.2, 0.15]),
                                           time_jumps = [200, 800], in_speed = torch.tensor(50.0),
                                           L = L)
        # Setting max_dens equal to 4 so that maximal inflow is always ensured
        # A queue will be formed, but we do not care about this queue, so it is fine
        e18_west_bndry = ibc.boundary_conditions(1, max_dens = 4, densities = torch.tensor([0.5]),
                                           time_jumps = [], in_speed = torch.tensor(80.0),
                                           L = L)
        e18_east_bndry = ibc.boundary_conditions(1, max_dens = 4, densities = torch.tensor([0.5]),
                                           time_jumps = [], in_speed = torch.tensor(80.0),
                                           L = L)
        boundary_fncs = [hw_bndry, festning_bndry, lundsbro_bndry, e18_west_bndry, e18_east_bndry]
    else:
        boundary_fncs = []
        hw_config = boundary_configs[0]
        hw_bndry = ibc.boundary_conditions(hw_config["type"], max_dens=hw_config["max_dens"], 
        ...
        lundsbro_bndry = ibc.boundary_conditions(lundsbro_config["type"], max_dens=lundsbro_config["max_dens"], 
                                           ...
                                           time_jumps = lundsbro_config["jumps"], in_speed = torch.tensor(lundsbro_config["in_speed"]),
                                           L = L)
        e18_west_config = boundary_configs[3]
        e18_west_bndry = ibc.boundary_conditions(e18_west_config["type"], max_dens=e18_west_config["max_dens"], 
                                           densities = torch.tensor(e18_west_config["densities"]),
                                           time_jumps = e18_west_config["jumps"], in_speed = torch.tensor(e18_west_config["in_speed"]),
                                           L = L)
        e18_east_config = boundary_configs[4]
        e18_east_bndry = ibc.boundary_conditions(e18_east_config["type"], max_dens=e18_east_config["max_dens"], 
                                           densities = torch.tensor(e18_east_config["densities"]),
                                           time_jumps = e18_east_config["jumps"], in_speed = torch.tensor(e18_east_config["in_speed"]),
                                           L = L)
        boundary_fncs = [hw_bndry, festning_bndry, lundsbro_bndry, e18_west_bndry, e18_east_bndry]
    ...
    # We add a lot of simplifications of the connection of e18 into kvadraturen
    # We consider about 200 metres to the left of kvadraturen
    # After about 100 metres we add a split into vesterveien that will lead into the roundabout north of vestre
    # strandgate
    # The e18 continues for 200 metres after the split, before leading to a junction that leads back 100 metres to
    # the same roundabout
    # e18 then continues for about 200 metres (180 and 220) before the two directions lead into two separate junctions leading into
    # the roundabout north of festningsgata
    # Finally we model about 200 metres of e18 to the east of kvadraturen.

    # Initialize container for the roads
    e18_fw = [None for _ in range(4)]
    e18_bw = [None for _ in range(4)]
    e18_west_out_fw = [None for _ in range(2)]
    e18_west_out_bw = [None for _ in range(2)]
    e18_east_out_fw = [None for _ in range(2)]
    e18_east_out_bw = [None for _ in range(2)]

    # Setting up the speed limits:
    if torch.is_tensor(e18_speeds[0]):
        e18_speed_limit = [v / 3.6 for v in e18_speeds]
    else:
        e18_speed_limit = [torch.tensor(v / 3.6) for v in e18_speeds]
    for v in e18_speed_limit:
        v.requires_grad = track_grad
    
    if torch.is_tensor(e18_w_1_speeds[0]):
        e18_w_1_speed_limit = [v / 3.6 for v in e18_w_1_speeds]
    else:
        e18_w_1_speed_limit = [torch.tensor(v / 3.6) for v in e18_w_1_speeds]
    for v in e18_w_1_speed_limit:
        v.requires_grad = track_grad
    
    if torch.is_tensor(e18_w_2_speeds[0]):
        e18_w_2_speed_limit = [v / 3.6 for v in e18_w_2_speeds]
    else:
        e18_w_2_speed_limit = [torch.tensor(v / 3.6) for v in e18_w_2_speeds]
    for v in e18_w_2_speed_limit:
        v.requires_grad = track_grad
    
    if torch.is_tensor(e18_e_1_speeds[0]):
        e18_e_1_speed_limit = [v / 3.6 for v in e18_e_1_speeds]
    else:
        e18_e_1_speed_limit = [torch.tensor(v / 3.6) for v in e18_e_1_speeds]
    for v in e18_e_1_speed_limit:
        v.requires_grad = track_grad
    
    if torch.is_tensor(e18_e_2_speeds[0]):
        e18_e_2_speed_limit = [v / 3.6 for v in e18_e_2_speeds]
    else:
        e18_e_2_speed_limit = [torch.tensor(v / 3.6) for v in e18_e_2_speeds]
    for v in e18_e_2_speed_limit:
        v.requires_grad = track_grad
...
def create_roads_w_e18_2(N = 2, v_strand_speeds = [torch.tensor(50.0)], v_strand_controls = [],
                        h_w_speeds = [torch.tensor(30.0)], h_w_controls = [],
                        tollbod_speeds = [torch.tensor(30.0)], tollbod_controls = [],
                        elvegate_speeds = [torch.tensor(30.0)], elvegate_controls = [],
                        dronning_speeds = [torch.tensor(50.0)], dronning_controls = [],
    ...
    for i in range(6):
        if i == 0:
            # Combine first three to form one road
            # b = 3 since three roads are combined
            v_strand_fw[0] = rd.Road(3, L, N, v_strand_speed_limit, v_strand_control_points, left_pos=(0, 0), right_pos=(0, 3-offset),
                                     id="v_strand_" + str(1) + "fw", initial=initial_fncs[0], boundary_fnc=None, max_dens=2)
            v_strand_bw[0] = rd.Road(3, L, N, v_strand_speed_limit, v_strand_control_points, left_pos=(0, 3-offset), right_pos=(0, 0),
                                     id="v_strand_" + str(1) + "bw",initial=initial_fncs[0], boundary_fnc=None, max_dens=2)
        else:
            v_strand_fw[i] = rd.Road(1, L, N, v_strand_speed_limit, v_strand_control_points, left_pos=(0, i+2+offset), right_pos=(0, i+3-offset),
                                     id="v_strand_" + str(i+1) + "fw",initial=initial_fncs[0], boundary_fnc=None, max_dens=2)
            v_strand_bw[i] = rd.Road(1, L, N, v_strand_speed_limit, v_strand_control_points, left_pos=(0, i+3-offset), right_pos=(0, i+2+offset),
                                     id="v_strand_" + str(i+1) + "bw",initial=initial_fncs[0], boundary_fnc=None, max_dens=2)
    ...
    h_w[0] = rd.Road(1, L, N, h_w_speed, h_w_control_points, left_pos=(-0.5, 3), right_pos=(0-offset, 3),
                     id="h_w_" + str(1),initial=initial_fncs[1], boundary_fnc=boundary_fncs[0])
    h_w[1] = rd.Road(6, L, N, h_w_speed, h_w_control_points, left_pos=(0+offset, 3), right_pos=(3-offset+6*tilt, 3),
                     id="h_w_" + str(2),initial=initial_fncs[1], boundary_fnc=None)
    ...
    for v in tollbod_speed:
        v.requires_grad = track_grad
    tollbod_control_points = tollbod_controls
    tollbod_bw[0] = rd.Road(6, L, N, tollbod_speed, tollbod_control_points, left_pos=(3-offset+2*tilt, 7.1), right_pos=(0+offset, 7.1),
                            id="tollbod_" + str(1) + "bw", initial=initial_fncs[2], boundary_fnc=None)
    tollbod_bw[1] = rd.Road(6, L, N, tollbod_speed, tollbod_control_points, left_pos=(6-offset, 7), right_pos=(3+offset+2*tilt, 7),
                            id="tollbod_" + str(2) + "bw", initial=initial_fncs[2], boundary_fnc=None)
    tollbod_fw[1] = rd.Road(6, L, N, tollbod_speed, tollbod_control_points, left_pos=(3+offset+2*tilt, 7), right_pos=(6-offset, 7),
                            id="tollbod_" + str(2) + "fw", initial=initial_fncs[2], boundary_fnc=None)
    ...
    for v in elvegate_speed:
        v.requires_grad = track_grad
    elvegate_control_points = elvegate_controls
    elvegata_fw = [rd.Road(1, L, N, elvegate_speed, elvegate_control_points, left_pos=(6, 7+offset), right_pos=(6, 8-offset),
                           id="elvegata_fw", initial=initial_fncs[3], boundary_fnc=None)]
    elvegata_bw = [rd.Road(1, L, N, elvegate_speed, elvegate_control_points, left_pos=(6, 8-offset), right_pos=(6, 7+offset),
                           id="elvegata_bw", initial=initial_fncs[3], boundary_fnc=None)]
    ...
    for v in dronning_speed:
        v.requires_grad = track_grad
    dronning_control_points = dronning_controls
    dronning_fw[0] = rd.Road(4, L, N, dronning_speed, dronning_control_points, left_pos=(0+offset, 8), right_pos=(2-offset, 8),
                             id="dronning_" + str(1) + "fw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    dronning_bw[0] = rd.Road(4, L, N, dronning_speed, dronning_control_points, left_pos=(2-offset, 8), right_pos=(0+offset, 8),
                             id="dronning_" + str(1) + "bw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    dronning_fw[1] = rd.Road(2, L, N, dronning_speed, dronning_control_points, left_pos=(2+offset, 8), right_pos=(3-offset, 8),
                             id="dronning_" + str(2) + "fw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    dronning_bw[1] = rd.Road(2, L, N, dronning_speed, dronning_control_points, left_pos=(3-offset, 8), right_pos=(2+offset, 8),
                             id="dronning_" + str(2) + "bw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    dronning_fw[2] = rd.Road(6, L, N, dronning_speed, dronning_control_points, left_pos=(3+offset, 8), right_pos=(6-offset, 8),
                             id="dronning_" + str(3) + "fw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    dronning_bw[2] = rd.Road(6, L, N, dronning_speed, dronning_control_points, left_pos=(6-offset, 8), right_pos=(3+offset, 8),
                             id="dronning_" + str(3) + "bw", initial=initial_fncs[4], boundary_fnc=None, max_dens=1)
    ...
    for i in range(6):
        if i == 0:
            festning_fw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(9-i)*tilt, i+offset),
                                     right_pos=(3+(8-i)*tilt, i+1-offset), id="festning_" + str(i+1) + "fw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
            festning_bw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(8-i)*tilt, i+1-offset),
                                     right_pos=(3+(9-i)*tilt, i+offset), id="festning_" + str(i+1) + "bw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
        elif i == 1:
            festning_fw[i] = rd.Road(2, L, N, festning_speed, festning_control_points, left_pos=(3+(9-i)*tilt, i+offset),
            ...
            festning_fw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(8-i)*tilt, i+1+offset),
                                     right_pos=(3+(7-i)*tilt, i+2-offset), id="festning_" + str(i+1) + "fw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
            festning_bw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(7-i)*tilt, i+2-offset),
                                     right_pos=(3+(8-i)*tilt, i+1+offset), id="festning_" + str(i+1) + "bw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
        elif i == 3:
            festning_fw[i] = rd.Road(3, L, N, festning_speed, festning_control_points, left_pos=(3+(8-i)*tilt, i+1+offset),
                                     right_pos=(3+(5-i)*tilt, i+4-offset), id="festning_" + str(i+1) + "fw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
            festning_bw[i] = rd.Road(3, L, N, festning_speed, festning_control_points, left_pos=(3+(5-i)*tilt, i+4-offset),
                                     right_pos=(3+(8-i)*tilt, i+1+offset), id="festning_" + str(i+1) + "bw",
                                     initial=initial_fncs[5], boundary_fnc=None,max_dens=2)
        elif i == 4:
            # i = 4
            # i = 5
            festning_fw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(6-i)*tilt, i+3+offset),
                                     right_pos=(3+(5-i)*tilt, i+4-offset), id="festning_" + str(i+1) + "fw",
                                     initial=initial_fncs[5],boundary_fnc=None, max_dens=2)
            festning_bw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(5-i)*tilt, i+4-offset),
                                     right_pos=(3+(6-i)*tilt, i+3+offset), id="festning_" + str(i+1) + "bw",
                                     initial=initial_fncs[5],boundary_fnc=None, max_dens=2)
        else:
            festning_fw[i] = rd.Road(1, L, N, festning_speed, festning_control_points, left_pos=(3+(6-i)*tilt, i+3+offset),
    ...
    lundsbro_fw = [rd.Road(2, L, N, lundsbro_speed, lundsbro_control_points, left_pos=(6+offset, 8), right_pos=(7, 7.8),
                           id="lundsbro_fw", initial=initial_fncs[6], boundary_fnc=None)]
    lundsbro_bw = [rd.Road(2, L, N, lundsbro_speed, lundsbro_control_points, left_pos=(7, 7.8), right_pos=(6+offset, 8),
                           id="lundsbro_bw", initial=initial_fncs[6], boundary_fnc=boundary_fncs[2], max_dens=2)]
    ...
    # First we add the first 100 metres, including west incoming boundary conditions
    e18_fw[0] = rd.Road(2, L, N, e18_speed_limit, e18_controls, left_pos=(-2, -0.25), right_pos=(-1, -1),
                        id="e18_1fw", initial=initial_fncs[7], boundary_fnc=boundary_fncs[3],
                        max_dens = 2)
    e18_bw[0] = rd.Road(2, L, N, e18_speed_limit, e18_controls, left_pos=(-1, -1), right_pos=(-2, -0.25),
                        id="e18_1bw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    # Next we add the 200 metres to the left right of the vestre strandgate roundabout
    e18_fw[1] = rd.Road(4, L, N, e18_speed_limit, e18_controls, left_pos=(-1, -1), right_pos=(1, -1),
                        id="e18_2fw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    e18_bw[1] = rd.Road(4, L, N, e18_speed_limit, e18_controls, left_pos=(1, -1), right_pos=(-1, -1),
                        id="e18_2bw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    # Next we add the 150 and 250 metres between the two roundabouts
    e18_fw[2] = rd.Road(5, L, N, e18_speed_limit, e18_controls, left_pos=(1, -1), right_pos=(3+9*tilt+0.75, -1.5),
                        id="e18_3fw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    e18_bw[2] = rd.Road(3, L, N, e18_speed_limit, e18_controls, left_pos=(3+9*tilt-0.75, -2), right_pos=(1, -1),
                        id="e18_3bw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    # Finally we add the last 250 and 150 metres of e18 that we model
    e18_fw[3] = rd.Road(3, L, N, e18_speed_limit, e18_controls, left_pos=(3+9*tilt+0.75, -1.5), right_pos=(7, -1),
                        id="e18_4fw", initial=initial_fncs[7], boundary_fnc=None, max_dens = 2)
    e18_bw[3] = rd.Road(5, L, N, e18_speed_limit, e18_controls, left_pos=(7, -1), right_pos=(3+9*tilt-0.75, -2),
                        id="e18_4bw", initial=initial_fncs[7], boundary_fnc=boundary_fncs[4], max_dens = 2)
    
    # Now we add the road sections that enter and leave e18:
    # Adding roads leading into vestre strandgate roundabout from the west
    e18_west_out_fw[0] = rd.Road(2, L, N, e18_w_1_speed_limit, e18_w_1_controls, left_pos=(-1, -1), right_pos=(-0.13, -0.3),
                                 id="e18_w_infw", initial=initial_fncs[8], boundary_fnc=None, max_dens=1)
    e18_west_out_bw[0] = rd.Road(2, L, N, e18_w_1_speed_limit, e18_w_1_controls, left_pos=(-0.13, -0.3), right_pos=(-1, -1),
                                 id="e18_w_inbw", initial=initial_fncs[8], boundary_fnc=None, max_dens=1)
    # Adding roads leading into vestre strandgate roundabout from the east
    e18_west_out_fw[1] = rd.Road(2, L, N, e18_w_2_speed_limit, e18_w_2_controls, left_pos=(0.27, -0.25), right_pos=(1, -1),
                                 id="e18_w_outfw", initial=initial_fncs[9], boundary_fnc=None, max_dens=1)
    e18_west_out_bw[1] = rd.Road(2, L, N, e18_w_2_speed_limit, e18_w_2_controls, left_pos=(1, -1), right_pos=(0.27, -0.25),
                                 id="e18_w_outbw", initial=initial_fncs[9], boundary_fnc=None, max_dens=1)
    # Adding roads leading into festningsgate roundabout from the west
    e18_east_out_fw[0] = rd.Road(1, L, N, e18_e_1_speed_limit, e18_e_1_controls, left_pos=(3+9*tilt-0.75, -2), right_pos=(3+9*tilt-0.16, offset-0.51),
                                 id="e18_e_infw", initial=initial_fncs[10], boundary_fnc=None, max_dens=1)
...
def create_roundabouts_w_e18_2(v_strand_fw, v_strand_bw, festning_fw, festning_bw,
                             e18_west_out_fw, e18_west_out_bw, e18_east_out_fw, e18_east_out_bw,
                       speed1 = [torch.tensor(50.0)], speed2 = [torch.tensor(50.0)], 
                       controls1 = [], controls2 = [], N = 2,
                       initial_densities = None, inflow_configs = None, track_grad = True):
...
def convert_junction_input_w_e18(vs_cycles, hw_cycle, tollbod_cycle, dronning_cycles, 
                           festning_cycles, track_grad):
    if track_grad:
        # Set requires grad = True for all cycle times:
        new_vs = []
...
def create_minimal_junctions_w_params_w_18(v_strand_fw, v_strand_bw, h_w, tollbod_fw, tollbod_bw,
                                      elvegata_fw, elvegata_bw, dronning_fw, dronning_bw,
                                      festning_fw, festning_bw, lundsbro_fw, lundsbro_bw,
                                      e18_fw, e18_bw, e18_west_out_fw, e18_west_out_bw,
    ...
    # Junctions for e18:
    e18_jncs = [None for _ in range(4)]
    # Westmost junction - no traffic light - no crossing connections, but row needed
    distribution = [[0.0, 0.8, 0.2],
                    [0.9, 0.0, 0.1],
                    [0.7, 0.3, 0.0]]
    priorities = [[0, 1, 1],
                  [1, 0, 2],
    ...
    crossings = [[[], [], []],
                 [[], [], []],
                 [[], [], []]]
    e18_jncs[0] = jn.Junction([e18_fw[0], e18_bw[0], e18_fw[1], e18_bw[1],
                               e18_west_out_fw[0], e18_west_out_bw[0]],
                               [0,3,5], [1,2,4], distribution, [], [],
                               duty_to_gw=True, priorities=priorities,
                               crossing_connections=crossings)
    # Junction between roads going out from vestre strandgate roundabout and e18
    distribution = [[0.0, 0.9, 0.1],
                    [0.8, 0.0, 0.2],
                    [0.3, 0.7, 0.0]]
    priorities = [[0, 1, 2],
                  [1, 0, 1],
    ...
    crossings = [[[], [], []],
                 [[], [], []],
                 [[], [], []]]
    e18_jncs[1] = jn.Junction([e18_fw[1], e18_bw[1], e18_fw[2], e18_bw[2],
                               e18_west_out_fw[1], e18_west_out_bw[1]],
                               [0,3,4], [1,2,5], distribution, [], [],
                               duty_to_gw=True, priorities=priorities,
                               crossing_connections=crossings)
    # Junction between westgoing e18 and roads going into festningsgate roundabout
    ...
    crossings = [[[], []],
                 [[], []]]
    e18_jncs[2] = jn.Junction([e18_bw[2], e18_bw[3],
                               e18_east_out_fw[0], e18_east_out_bw[0]],
                               [1,3], [0,2], distribution, [], [],
                               duty_to_gw=True, priorities=priorities,
                               crossing_connections=crossings)
    # Junction between eastgoing e18 and roads going into festningsgate roundabout
    distribution = [[0.8, 0.2],
                    [1.0, 0.0]]
    priorities = [[1,1],
                  [2,0]]
    crossings = [[[], []],
                 [[], []]]
    e18_jncs[3] = jn.Junction([e18_fw[2], e18_fw[3],
                               e18_east_out_fw[1], e18_east_out_bw[1]],
                               [0,2], [1,3], distribution, [], [],
                               duty_to_gw=True, priorities=priorities,
                               crossing_connections=crossings)

    return v_strand_jncs, h_w_jncs, tollbod_jncs, dronning_jncs, festning_jncs, e18_jncs
...
def generate_kvadraturen_w_e18(T = 30):
    # v_strand_fw, v_strand_bw, h_w, tollbod_fw, tollbod_bw, elvegata_fw, \
    # elvegata_bw, dronning_fw, dronning_bw, festning_fw, festning_bw, \
    ...
    elvegata_bw, dronning_fw, dronning_bw, festning_fw, festning_bw, \
    lundsbro_fw, lundsbro_bw, e18_fw, e18_bw, e18_west_out_fw, \
    e18_west_out_bw, e18_east_out_fw, e18_east_out_bw = create_roads_w_e18_2(track_grad=False)
    ...
    mainline_roads, roundabouts = create_roundabouts_w_e18_2(v_strand_fw[0], v_strand_bw[0], festning_fw[0], festning_bw[0],
                                                           e18_west_out_fw, e18_west_out_bw, e18_east_out_fw,
                                                           e18_east_out_bw, track_grad=False)
...
def generate_kvadraturen_from_config_e18(T, N, speed_limits, control_points, cycle_times, config = None, track_grad = True):
    ...
    ################################################
    # Changed to calling create_roads_w_e18_2 and create_roundabout_w_e18_2!!!!!
    ################################################
    ...
    elvegata_bw, dronning_fw, dronning_bw, festning_fw, festning_bw, \
    lundsbro_fw, lundsbro_bw, e18_fw, e18_bw, e18_west_out_fw, \
    e18_west_out_bw, e18_east_out_fw, e18_east_out_bw = create_roads_w_e18_2(N, speed_limits[0], control_points[0],
                                                                            speed_limits[1], control_points[1],
                                                                            speed_limits[2], control_points[2],
                                                                            speed_limits[3], control_points[3],
                                                                            speed_limits[4], control_points[4],
