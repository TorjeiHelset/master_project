...
class Road:
    ...
    def __init__(self, b, L, N, Vmax = 1., scheme = 3, limiter = "minmod", initial = lambda x: torch.zeros_like(x), inflow = -1):
        ...
        self.b = b
        self.L = L 
        self.dx = self.L / (N + 1) # Should be the same for all roads

        # Vmax is parameter that optimization should done wrt.
        # Therefore we set requires_grad to True
        self.Vmax = torch.tensor([float(Vmax)], requires_grad=True)
    ...
    def max_dt(self):
        ...
        return float(CFL * self.dx / (self.gamma * torch.max(torch.tensor([torch.abs(fv.d_flux(self.rho[j])) for j in range(len(self.rho))]))))
    ...
    def solve_internally(self, dt):
        ...
        match self.scheme:
            case 0:
                ...
                # print("Internal Flux")
                # print(F)
                self.rho[self.pad:-self.pad] -= self.gamma * dt/self.dx * (F[self.pad:] - F[:-self.pad])
            case 1:
                # Rusanov scheme
...
class Junction:
        ...
        for i in range(n):
            for j in range(m):
                if not active_idx[i,j]:
                    fluxes[i,j] = min(self.distribution[j] * gamma_in[i] * fv.D(rho_in[i].clone()),
                                    self.priority[i]* gamma_out[j] * fv.S(rho_out[j].clone()))
    ...
    def apply_bc(self, dt, t):
        ...
        for i, flux in enumerate(fluxes_in):
            ...
            s = torch.max(torch.abs(fv.d_flux(left)), torch.abs(fv.d_flux(in_mid)))
            left_f = fv.flux(left.clone())
            mid_f = fv.flux(in_mid.clone())
            ...
            # gamma
            road.rho[-road.pad] = road.rho[-road.pad] - dt/road.dx * (flux - road.gamma*left_flux)
            if road.pad > 1:
        ...
        for i, flux in enumerate(fluxes_out):
            ...
            s = torch.max(torch.abs(fv.d_flux(out_mid)), torch.abs(fv.d_flux(right)))
            mid_f = fv.flux(out_mid.clone(), road.Vmax)
            right_f = fv.flux(right.clone(), road.Vmax)
            right_flux = 0.5 * (mid_f + right_f) - 0.5 * s * (right - out_mid)
            #print(right_flux)
            #print(right_flux - flux)

            road.rho[road.pad-1] = road.rho[road.pad-1] - dt / road.dx * (road.gamma*right_flux - flux)
            if road.pad > 1:
...
def objective_function(network, type = 1):
    # Calculate some objective function
    # First, just density at some 

    match type:
        case 0:
            # Density at some point on first road in system at final time
            road = network.roads[0]
            out = road.rho[int(2*road.N/3)]
...
class RoadNetwork:
    ...
    def solve_cons_law(self):
        ...
        # objective = objective_function(self, self.object_type)
        # objective.backward()
        # #first_derivative = torch.autograd.grad(objective, self.roads[0].Vmax)[0]
        # #print(first_derivative)
        # for road in self.roads:
        #     print(road.Vmax.grad)
        return history_of_network#, first_derivative, second_derivative
