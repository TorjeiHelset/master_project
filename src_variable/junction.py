...
class RoundaboutJunction:
    ...
    def divide_flux(self, t):
        ...
        if not self.queue_junction:
            # All roads are a part of the simulation
            main_rho_in = self.mainline_in.rho[-self.mainline_in.pad]
            second_rho_in = self.secondary_in.rho[-self.secondary_in.pad]
            main_rho_out = self.mainline_out.rho[-self.mainline_out.pad]
            second_rho_out = self.secondary_out.rho[-self.secondary_out.pad]
            main_max_dens = self.mainline_in.max_dens
            second_max_dens = self.secondary_in.max_dens
            main_gamma = self.mainline_in.gamma[self.mainline_in.idx]
            second_gamma = self.secondary_in.gamma[self.secondary_in]

            # Calculating the demands:
            demands = torch.zeros(2)
            demands[0] = main_max_dens * fv.D(main_rho_in, main_gamma)
            demands[1] = second_max_dens * fv.D(second_rho_in, second_gamma)

            # Calculating the supplies
            supplies = torch.zeros(2)
            supplies[0] = main_max_dens * fv.S(main_rho_out, main_gamma)
            supplies[1] = second_max_dens * fv.S(second_rho_out, second_gamma)
