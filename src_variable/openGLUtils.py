...
def create_density_container_with_shift(network, densities):
    # Create a container for the densities
    # Every element should be the 
    colors = []
    points = []
    ...
    for t in times:
        ...
        for i, d in enumerate(density):
            ...
            left, right = shift_single_point(road.id, road.left_pos, road.right_pos)
            n = len(colors[-1][i])
            x = np.linspace(left[0], right[0], n)
            y = np.linspace(left[1], right[1], n)
            points[-1][i] = [(x[i], y[i]) for i in range(len(x))]
