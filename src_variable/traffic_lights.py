...
'''
Should maybe have a base class for traffic lights, and a subclass for each type of traffic light

Coupled traffic lights would then be a subclass that overwrites some methods, and 
traffic light with some approximation of right of way would be another subclass

In network, only methods of base class should be used.

The only method that should be used by the network is the get_influx method, which would
return the influx of cars across the junction at a given time t.
...
class TrafficLight:
    ...
    def get_state(self, t):
        ...
        Extend to take into account some delay
        '''

        # Time spent in current cycle
        tau = t % self.cycle_time 
        
        combined = 0
        for i in range(0, len(self.cycle), 2):
            # Only need to go through half of the intervals
            # If not in any of these, it will be in the other half, corresponding
            # to the other state
            combined += self.cycle[i].detach()
            if tau >= combined and tau < combined + self.cycle[i+1].detach():
                return not self.starting_state
            
        return self.starting_state
        
    
class CoupledTrafficLight:
    '''
    ...
    starting_state = None # If true, a light active, if false, b light active
    a_entering = []
    a_leaving = []
    b_entering = []
    b_leaving = []
    ...
    def __init__(self, starting_state, a_in, a_out, b_in, b_out, cycle, delaytime=0):
        self.starting_state = starting_state
